...
DROP TABLE IF EXISTS aiModelCategory;

-- 创建AI模型分类表（小驼峰命名）
CREATE TABLE aiModelCategory (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '分类ID',
    categoryKey VARCHAR(50) NOT NULL UNIQUE COMMENT '分类键值',
...
CREATE TABLE aiModel (
    ...
    isOfficial TINYINT(1) DEFAULT 0 COMMENT '是否官方模型：0-否，1-是',
    isActive TINYINT(1) DEFAULT 1 COMMENT '是否启用：0-禁用，1-启用',
    sortOrder INT DEFAULT 0 COMMENT '排序权重，数值越大越靠前',
    usageCount BIGINT DEFAULT 0 COMMENT '使用次数',
    creatorId BIGINT COMMENT '创建者ID',
    createTime DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updateTime DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    isDeleted TINYINT(1) DEFAULT 0 COMMENT '是否删除：0-否，1-是',

    INDEX idx_category (category),
    INDEX idx_isActive (isActive),
    INDEX idx_isOfficial (isOfficial),
    INDEX idx_sortOrder (sortOrder),
...
-- 插入分类数据
INSERT INTO aiModelCategory (categoryKey, categoryName, description, sortOrder, isActive) VALUES
('recommend', '官方推荐', '官方精选推荐的优质AI模型', 100, 1),
('office', '办公提效', '提升办公效率的AI助手', 90, 1),
('writing', '辅助写作', '帮助写作创作的AI工具', 80, 1),
('life', '生活实用', '解决生活问题的AI助手', 70, 1),
('education', '教育学习', '教育和学习相关的AI模型', 60, 1),
('entertainment', '娱乐休闲', '娱乐和休闲相关的AI助手', 50, 1);

-- 插入AI模型数据
INSERT INTO aiModel (name, description, icon, category, prompt, source, tags, isOfficial, isActive, sortOrder, creatorId) VALUES
...
-- 最终验证
SELECT '=== 最终验证 - 分类表 ===' as info;
SELECT id, categoryKey, categoryName, description, sortOrder, isActive FROM aiModelCategory ORDER BY sortOrder DESC;

SELECT '=== 最终验证 - 模型表 ===' as info;
SELECT id, name, category, isOfficial, isActive, sortOrder, usageCount FROM aiModel ORDER BY sortOrder DESC;
